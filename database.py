...
# Enables row access by column name and enforces foreign key constraints
def get_db():
...
def init_db():
    ...
    # Seed demo data (only runs if teacher does not already exist)
    ...
    if not cursor.fetchone():
        ...
        for uname, fname, snum, course, year in students:
            # Insert user account for each student
            cursor.execute(
        ...
        for code, name, tid in modules:
            cursor.execute(
                "INSERT INTO modules (code, name, teacher_id) VALUES (?,?,?)",
                (code, name, tid)
            )
        ...
        # Predefined score patterns for each student
        scores_map = {
        ...
        for si, sid in enumerate(student_ids):
            for mi, mid in enumerate(module_ids):
                for ai, assessment in enumerate(assessments):
                    ...
                    cursor.execute(
                        "INSERT INTO grades (student_id, module_id, assessment_name, score, max_score, date) VALUES (?,?,?,?,?,?)",
        ...
        for si, sid in enumerate(student_ids):
            for mi, mid in enumerate(module_ids):
                for week in range(12):
                    ...
                    # Simulate poorer attendance for some students
                    if si in [1, 4]:  # Bob and Eve
